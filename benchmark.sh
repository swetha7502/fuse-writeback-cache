...
UNDERLYING=/tmp/fuse_underlying
MOUNTPOINT=/tmp/fuse_mount
BINARY=./myfs_cache
RESULTS_DIR=./results

mkdir -p "$UNDERLYING" "$MOUNTPOINT" "$RESULTS_DIR"
...
run_bench() {
  ...
  --output-format=json > "$RESULTS_DIR/${MODE}_seqwrite.json"
  ...
  --output-format=json > "$RESULTS_DIR/${MODE}_randwrite.json"

    # # Sequential read (tests cache behavior)
